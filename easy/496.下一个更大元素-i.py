...
class Solution:
    def OnenextGreaterElement(self, nums1: List[int]) -> List[int]:
        n = len(nums1)
        res = [0]*n
        stack = []
        for i in range(n-1, -1, -1):
            while len(stack) != 0 and nums1[i] >= stack[-1]:
                stack.pop()
    ...
    def nextGreaterElement(self, nums1: List[int], nums2: List[int]) -> List[int]:
        n2l = len(nums2)
        res = []
        greater = self.OnenextGreaterElement(nums2)
        print(greater)
        greater_map = {}
        for i in range(n2l):
            greater_map[nums2[i]] = greater[i]
        for n1 in nums1:
            res.append(greater_map.get(n1))
        return res
