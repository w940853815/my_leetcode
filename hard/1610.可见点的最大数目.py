    ...
    def visiblePoints(
        ...
        for p in points:
            if p == location:
                ...
                angle_list.append(atan2(p[0] - location[0], p[1] - location[1]))
        angle_list.sort()
        _len = len(angle_list)
        angle_list += [a + 2 * pi for a in angle_list]
        deg = angle * pi / 180
        right = 0

        for i in range(_len):
            while angle_list[right] <= angle_list[i] + deg and right < _len * 2:
                right += 1
            max_cnt = max(right - i, max_cnt)

        return max_cnt + same_cnt
...
if __name__ == "__main__":
    s = Solution()
    # res = 4
    # res = s.visiblePoints([[2, 1], [2, 2], [3, 4], [1, 1]], 90, [1, 1])
    # res = 2
    res = s.visiblePoints([[1, 1], [2, 2], [1, 2], [2, 1]], 0, [1, 1])
    print(res)