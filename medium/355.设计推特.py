...
class Twitter:
    ...
    def __init__(self):
        ...
        self.__post = defaultdict(list)
        self.time = 0

    def postTweet(self, userId: int, tweetId: int) -> None:
        self.__post[userId].append((-self.time, (userId, tweetId)))
        self.time += 1

    def getNewsFeed(self, userId: int) -> List[int]:
        news = []
        followeeIds = [userId]
        followeeIds.extend(self.__follow[userId])
        for fi in followeeIds:
            news.extend(self.__post[fi])
        heapq.heapify(news)
        return [i[1][1] for i in heapq.nsmallest(10, news)]

    def follow(self, followerId: int, followeeId: int) -> None:
        if followeeId not in self.__follow[followerId]:
            self.__follow[followerId].append(followeeId)

    def unfollow(self, followerId: int, followeeId: int) -> None:
        if followeeId in self.__follow[followerId]:
...
if __name__ == '__main__':
    twitter = Twitter()
    print(twitter.postTweet(1, 5))  # 用户 1 发送了一条新推文 (用户 id = 1, 推文 id = 5)
    print(twitter.getNewsFeed(1))  # 用户 1 的获取推文应当返回一个列表，其中包含一个 id 为 5 的推文
    print(twitter.follow(1, 2))    # 用户 1 关注了用户 2
    print(twitter.postTweet(2, 6))  # 用户 2 发送了一个新推文 (推文 id = 6)
    # 用户 1 的获取推文应当返回一个列表，其中包含两个推文，id 分别为 -> [6, 5] 。推文 id 6 应当在推文 id 5 之前，因为它是在 5 之后发送的
    print(twitter.getNewsFeed(1))
    print(twitter.unfollow(1, 2))  # 用户 1 取消关注了用户 2
