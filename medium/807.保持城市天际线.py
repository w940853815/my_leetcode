...
class Solution:
    def maxIncreaseKeepingSkyline(self, grid: List[List[int]]) -> int:
        _len = len(grid)
        trans_grid = [[0 for _ in range(_len)] for _ in range(_len)]
        res = [[0 for _ in range(_len)] for _ in range(_len)]
        grid_sum = 0
        res_sum = 0
        for i in range(_len):
            grid_sum += sum(grid[i])
        ...
        for i in range(_len):
            for j in range(_len):
                res[i][j] = min(max(grid[i]), max(trans_grid[j]))
                res_sum += res[i][j]
        # print(res)
        return res_sum - grid_sum


# @lc code=end
if __name__ == "__main__":
    s = Solution()
    res = s.maxIncreaseKeepingSkyline(
